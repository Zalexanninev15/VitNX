...
namespace VitNX3.UI.OtherControls
{
    public class FontAwesome4
    {
        ...
        {
            public string SetFontFileSaveLocation { get; set; }
            ...
            {
                SetFontFileSaveLocation = Default.SetFontFileSaveLocation;
                Size = Default.Size;
        ...
        {
            var size = GetFontIconRealSize(props.Size,
                (int)props.Type);
            var bmpTemp = new Bitmap(size.Width,
                size.Height);
            using (Graphics g1 = Graphics.FromImage(bmpTemp))
            {
                ...
                g1.DrawString(character,
                    font,
                    new SolidBrush(props.ForeColor),
                    0, 0);
                g1.DrawImage(bmpTemp,
                    0, 0);
            }
            var bmp = ResizeImage(bmpTemp,
                props);
            if (props.ShowBorder)
            {
        ...
        private Size GetFontIconRealSize(int size, int iconIndex)
        {
            ...
            using (Graphics g1 = Graphics.FromImage(bmpTemp))
            {
                ...
                var sizeF = g1.MeasureString(character,
                    font,
                    new Point(0, 0),
                    format);
                return sizeF.ToSize();
            ...
            Properties props)
        {
            ...
            Bitmap b = new Bitmap(props.Size + props.Location.X,
                props.Size + props.Location.Y);
        ...
        private void LoadFont()
        {
            try
            {
                if (!File.Exists(Properties.Default.SetFontFileSaveLocation)
                    && !string.IsNullOrEmpty(_downloadLink))
                {
                        ...
                        && (downloadUri.Scheme == Uri.UriSchemeHttp || downloadUri.Scheme == Uri.UriSchemeHttps))
                    {
                        using (var client = new System.Net.WebClient())
                            client.DownloadFile(downloadUri, Properties.Default.SetFontFileSaveLocation);
                    }
                }
            }
            finally
            {
                if (File.Exists(Properties.Default.SetFontFileSaveLocation))
                    _fonts.AddFontFile(Properties.Default.SetFontFileSaveLocation);
            }
        }
        ...
        private Font GetIconFont(int pixelSize)
        {
            ...
            var font = new Font(_fonts.Families[0],
                size,
                FontStyle.Regular,
                GraphicsUnit.Point);
            return font;
    ...
    {
        public static Bitmap StackWith(this Bitmap backgroundImage,
            FontAwesome4.Properties foregroundImage)
        {
        ...
        public static Bitmap AsImage(this FontAwesome4.Type type,
            FontAwesome4.Properties fontProperties = null)
        {
        ...
        public static Icon AsIcon(this FontAwesome4.Type type,
            FontAwesome4.Properties fontProperties = null)
        {
            if (fontProperties == null)
                fontProperties = FontAwesome4.Properties.Get(type);
