...
namespace VitNX.Functions
{
	public class ConsoleProgressBar : IDisposable, IProgress<double>
	{
		...
		ConsoleColor pb = new ConsoleColor();

		public ConsoleProgressBar() { timer = new Timer(TimerHandler); if (!Console.IsOutputRedirected) { ResetTimer(); } }
		public void Report(double value) { value = Math.Max(0, Math.Min(1, value)); Interlocked.Exchange(ref currentProgress, value); }
		public void SetColor(ConsoleColor my_pb) { pb = my_pb; }
		public void SetText(string work1) { work = work1; }
		public void NotUsed(bool pbd) { workdo = pbd; }
